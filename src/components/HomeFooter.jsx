...
export default function HomeFooter() {
  return (
    <footer className="bg-[#0B1F33] px-5 py-9 font-sans text-[#F7F8FB] sm:px-6 sm:py-10 lg:px-20">
      <div className="mx-auto max-w-7xl">
        <div className="mb-8 grid grid-cols-1 gap-8 md:grid-cols-[0.9fr_2.1fr] md:gap-10 lg:mb-10 lg:gap-12">
          <div>
            ...
            <p className="mb-5 max-w-sm text-[14px] font-light leading-6 text-[#F7F8FB]/72 sm:text-[15px] md:text-sm md:leading-6">
            ...
            <div className="font-sans text-[10px] font-bold uppercase tracking-[0.18em] text-[#B38F4F] sm:text-[11px]">
              Austin, TX
            </div>
          </div>

          <div className="grid min-w-0 grid-cols-3 gap-3 sm:gap-6 lg:gap-10">
            {Object.entries(footerGroups).map(([group, links]) => (
              <nav key={group} aria-label={group}>
                <h4 className="mb-3 font-sans text-[10px] font-bold uppercase tracking-[0.16em] text-[#B38F4F] sm:mb-4 sm:text-[11px]">
                  {group}
                </h4>
                <ul className="space-y-2 text-[14px] font-light leading-5 text-[#F7F8FB]/72 sm:space-y-2.5 sm:text-[15px]">
                  {links.map((link) => (
        ...
        <div className="flex flex-col items-center justify-center gap-2 pt-5 text-center font-sans text-[10px] font-bold uppercase tracking-[0.16em] text-[#F7F8FB]/46 sm:flex-row sm:gap-4 sm:text-[11px]">
